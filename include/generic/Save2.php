...
if (isset($_REQUEST['return_type'])  && $_REQUEST['return_type'] == 'report') {
    ...
    require_once($beanFiles[$bean_name]);
    $parent = new $bean_name();
    $parent->retrieve($_REQUEST['record']);
    ...
    if (isset($_REQUEST['subpanel_id'])  && !empty($_REQUEST['subpanel_id'])) {
 	...
 	if(!empty($_REQUEST['select_entire_list']) &&  $_REQUEST['select_entire_list'] != 'undefined' && isset($_REQUEST['current_query_by_page'])){
        ...
        if(empty($beanList[$module])) sugar_die($GLOBALS['app_strings']['ERROR_NO_BEAN']);
        $bean_name = $beanList[$module];
        if(empty($beanFiles[$bean_name])) sugar_die($GLOBALS['app_strings']['ERROR_NO_BEAN']);
        require_once($beanFiles[$bean_name]);
        $seed = new $bean_name();
