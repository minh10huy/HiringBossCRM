...
eoq;
	...
	function getCacheValue($ieId, $type, $file, $key) {
        ...
        $cacheFilePath = "{$sugar_config['cache_dir']}modules/Emails/{$cleanIeId}/{$cleanType}/{$cleanFile}";
		$cacheFile = array();
	...
	function writeCacheFile($key, $var, $ieId, $type, $file) {
		...
		$cleanType = cleanDirName($type);
		$cleanFile = cleanFileName($file);
		$the_file = clean_path("{$sugar_config['cache_dir']}/modules/Emails/{$cleanIeId}/{$cleanType}/{$cleanFile}");
