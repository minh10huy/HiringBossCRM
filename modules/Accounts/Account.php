	...
	* @deprecated.
	...
	//	$this->assigned_user_name = get_assigned_user_name($this->assigned_user_id);
		...
		if (!empty($this->contact_id)) {
			$query  = "SELECT first_name, last_name, title, phone_mobile, phone_work FROM contacts";
			$query .= "WHERE id='$this->contact_id'";
			$result = $this->db->limitQuery($query,0,1,true," Error filling in additional detail fields: ");
			// Get the contact name.
			$row = $this->db->fetchByAssoc($result);
			//$GLOBALS['log']->info("additional contact fields $query");
			if($row != null)
			{
				$this->contact_name = $locale->getLocaleFormattedName($row['first_name'], $row['last_name'], '', '');
				$this->title = $row['title'];
				$this->phone_mobile = $row['phone_mobile'];
...
//		$temp_array["ENCODED_NAME"]=htmlspecialchars($this->name, ENT_QUOTES);
        ...
        $accountContacts = $this->get_contacts();
        foreach ($accountContacts as $contact) {
        	//var_dump($contact);
        	if ($contact->title != null && trim($contact->title) != '') {
		        $temp_array['TITLE'] = $contact->title;
        	}
        	if ($contact->phone_work != null && trim($contact->phone_work) != ''){
        		$temp_array['PHONE_MOBILE'] = $contact->phone_mobile;
        	}
        	if ($contact->phone_mobile != null && trim($contact->phone_mobile) != ''){
        		 $temp_array['PHONE_MOBILE'] = $contact->phone_mobile;
        	}
        	//take contact name lead_source owner
        	$this->contact_id = $contact->id;
	        $query = "select first_name, last_name from contacts where lead_source in ('owner', 'ownercontact') and id='$this->contact_id'";
			$result = $this->db->query($query);
			$row = $this->db->fetchByAssoc($result);
			if($row != null){
				$temp_array['CONTACT_OWNER'] = $row['first_name'].' '.$row['last_name'];
			}
			//take contact name lead_source contact
			if(isset($_GET['saved'])){
        		$query = "select first_name, last_name from contacts where lead_source in ('contact', 'ownercontact') and id='$this->contact_id'";
				$result = $this->db->query($query);
				$row = $this->db->fetchByAssoc($result);
				if($row != null){
					$temp_array['CONTACT_COLDCALL'] = $row['first_name'].' '.$row['last_name'];
				}
			}
			//take email from email_address
	        $query  = "SELECT ea.email_address FROM email_addresses ea JOIN email_addr_bean_rel eabr ON ea.id=eabr.email_address_id ";
			$query .= "WHERE eabr.bean_id='$contact->id' and eabr.bean_module = 'Contacts' and eabr.deleted=0 and eabr.primary_address=1";
			//$result = $this->db->limitQuery($query,0,1,true," Error filling in additional detail fields: ");
			$result = $this->db->query($query);
			$row = $this->db->fetchByAssoc($result);
        	if($row != null)
			{
				$temp_array['EMAIL1'] = $row['email_address'];
			}
			//take user follow username for Lead Owner
			$query = "SELECT CONCAT(u.first_name,' ' ,u.last_name) AS full_name FROM users u LEFT JOIN contacts c ON u.id=c.assigned_user_id WHERE u.user_name in ('admin', 'manager') LIMIT 0,1";
        	$result = $this->db->query($query);
			$row = $this->db->fetchByAssoc($result);
        	if($row != null)
			{
				$temp_array['LEAD_OWNER'] = $row['full_name'];
			}
        	//take user follow username for Inside Sales
			$query = "SELECT CONCAT(u.first_name,' ' ,u.last_name) AS full_name FROM users u LEFT JOIN contacts c ON u.id=c.assigned_user_id WHERE u.user_name in ('sales', 'inside sales') LIMIT 0,1";
        	$result = $this->db->query($query);
			$row = $this->db->fetchByAssoc($result);
        	if($row != null)
			{
				$temp_array['INSIDE_SALES'] = $row['full_name'];
			}
			//take opportunities name
        	$query = "SELECT o.name, o.date_closed FROM opportunities o JOIN accounts_opportunities ao ON o.id=ao.opportunity_id WHERE ao.account_id='$this->id'";
        	$result = $this->db->query($query);
			$row = $this->db->fetchByAssoc($result);
        	if($row != null)
			{
				$temp_array['OPP_NAME'] = $row['name'];
				$temp_array['DATE_CLOSED'] = $row['date_closed'];
			}
			//take products, subcription fee, implementation fee, committed from opportunities_cstm
        	$query = "SELECT oc.products_c, oc.subcriptionfee_c, oc.implementationfee_c, oc.committed_c FROM opportunities_cstm oc JOIN opportunities o ON oc.id_c=o.id JOIN accounts_opportunities ao ON o.id=ao.opportunity_id WHERE ao.account_id = '$this->id'";
        	$result = $this->db->query($query);
			$row = $this->db->fetchByAssoc($result);
        	if($row != null)
			{
				$temp_array['PRODUCTS'] = $row['products_c'];
				$temp_array['SUBCRIPTION_FEE'] = $row['subcriptionfee_c'];
				$temp_array['IMPLEMENTATION_FEE'] = $row['implementationfee_c'];
				$temp_array['COMMITTED'] = $row['committed_c'];
			}
			//take startdate, enddate, finance contact person, hiring boss id, xero id
        	$query = "SELECT oc.startdate_c, oc.enddate_c, oc.financecp_c, oc.hiringbossid_c, oc.xeroid_c FROM opportunities_cstm oc JOIN opportunities o ON oc.id_c=o.id JOIN accounts_opportunities ao ON o.id=ao.opportunity_id WHERE ao.account_id = '$this->id'";
        	$result = $this->db->query($query);
			$row = $this->db->fetchByAssoc($result);
        	if($row != null)
			{
				$temp_array['START_DATE'] = $row['startdate_c'];
				$temp_array['END_DATE'] = $row['enddate_c'];
				$temp_array['FINANCE_CONTACT_PERSON'] = $row['financecp_c'];
				$temp_array['HIRINGBOSS_ID'] = $row['hiringbossid_c'];
				$temp_array['XERO_ID'] = $row['xeroid_c'];
			}
			//take Description of Cold Call
        	$query = "SELECT description FROM calls WHERE parent_id = '$this->id' ORDER BY date_entered DESC LIMIT 0,1";
        	$result = $this->db->query($query);
			$row = $this->db->fetchByAssoc($result);
        	if($row != null)
			{
				$temp_array['DESCRIPTION'] = $row['description'];
			}
			//die($this->id);
	        //break;
			//$GLOBALS['log']->info("additional contact fields $query");
        }
        //$temp_array['CONTACT_ID'] = $this->contact_id;
        //$temp_array['CONTACT_NAME'] .= $this->contact_name;
        $temp_array['NUM_OF_CONTACT'] = count($accountContacts);
		return $temp_array;
	}
	/**
