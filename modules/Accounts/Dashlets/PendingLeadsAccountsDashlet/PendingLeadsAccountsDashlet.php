...
class PendingLeadsAccountsDashlet extends DashletGeneric { 
    function PendingLeadsAccountsDashlet($id, $def = null) {
		global $current_user, $app_strings;
		require('modules/Accounts/Dashlets/PendingLeadsAccountsDashlet/PendingLeadsAccountsDashlet.data.php');

        parent::DashletGeneric($id, $def);

        if(empty($def['title'])) $this->title = 'Pending Leads';

        $this->searchFields = $dashletData['PendingLeadsAccountsDashlet']['searchFields'];
        $this->columns = $dashletData['PendingLeadsAccountsDashlet']['columns'];
	...
	function process($lvsParams = array()) {
		...
		$lvsParams = array(
	     		        ...
	     		        'custom_where' => ' AND (accounts.id in (select acstm.id_c from accounts_cstm acstm where acstm.onem_c in (\'onemforange\', \'onemsorange\')))',             
					); 
		}
