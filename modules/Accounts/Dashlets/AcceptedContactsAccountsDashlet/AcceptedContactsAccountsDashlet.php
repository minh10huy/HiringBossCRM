...
class AcceptedContactsAccountsDashlet extends DashletGeneric { 
    function AcceptedContactsAccountsDashlet($id, $def = null) {
		global $current_user, $app_strings;
		require('modules/Accounts/Dashlets/AcceptedContactsAccountsDashlet/AcceptedContactsAccountsDashlet.data.php');

        parent::DashletGeneric($id, $def);

        if(empty($def['title'])) $this->title = 'Accepted Contacts';

        $this->searchFields = $dashletData['AcceptedContactsAccountsDashlet']['searchFields'];
        $this->columns = $dashletData['AcceptedContactsAccountsDashlet']['columns'];
	...
	function process($lvsParams = array()) {
		...
		if(isset($_GET['saved']) and $_GET['saved'] == 'ee9f21d5'){
			$lvsParams = array(
		        'custom_where' => ' AND (accounts.id in (select acstm.id_c from accounts_cstm acstm where acstm.contact_c in (\'contactgreen\')))',             
			); 
		}
