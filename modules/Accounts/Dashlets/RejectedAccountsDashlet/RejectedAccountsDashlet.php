...
class RejectedAccountsDashlet extends DashletGeneric { 
    function RejectedAccountsDashlet($id, $def = null) {
		global $current_user, $app_strings;
		require('modules/Accounts/Dashlets/RejectedAccountsDashlet/RejectedAccountsDashlet.data.php');

        parent::DashletGeneric($id, $def);

        if(empty($def['title'])) $this->title = 'Rejected Accounts';

        $this->searchFields = $dashletData['RejectedAccountsDashlet']['searchFields'];
        $this->columns = $dashletData['RejectedAccountsDashlet']['columns'];
	...
	function process($lvsParams = array()) {
		...
		if(isset($_GET['saved']) and $_GET['saved'] == '71cb0f85'){
		$lvsParams = array(
	     				//'custom_from' => ' JOIN accounts_contacts ac ON ac.contact_id=contact.id',
	     		        'custom_where' => ' AND (accounts.id in (select acstm.id_c from accounts_cstm acstm where acstm.accsigned_c in (\'accfred\', \'accsred\')))',             
					);
		} 
