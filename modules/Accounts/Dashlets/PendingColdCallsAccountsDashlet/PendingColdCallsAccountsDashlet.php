...
class PendingColdCallsAccountsDashlet extends DashletGeneric { 
    function PendingColdCallsAccountsDashlet($id, $def = null) {
		global $current_user, $app_strings;
		require('modules/Accounts/Dashlets/PendingColdCallsAccountsDashlet/PendingColdCallsAccountsDashlet.data.php');

        parent::DashletGeneric($id, $def);

        if(empty($def['title'])) $this->title = 'Pending Cold Calls';

        $this->searchFields = $dashletData['PendingColdCallsAccountsDashlet']['searchFields'];
        $this->columns = $dashletData['PendingColdCallsAccountsDashlet']['columns'];
	...
	function process($lvsParams = array()) {
		...
		$lvsParams = array(
	     		        ...
	     		        'custom_where' => ' AND (accounts.id in (select acstm.id_c from accounts_cstm acstm where acstm.coldcalls_c in (\'ccorange\')))',             
					); 
		}
