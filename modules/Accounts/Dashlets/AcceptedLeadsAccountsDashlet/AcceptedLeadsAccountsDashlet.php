...
class AcceptedLeadsAccountsDashlet extends DashletGeneric { 
    function AcceptedLeadsAccountsDashlet($id, $def = null) {
		global $current_user, $app_strings;
		require('modules/Accounts/Dashlets/AcceptedLeadsAccountsDashlet/AcceptedLeadsAccountsDashlet.data.php');

        parent::DashletGeneric($id, $def);

        if(empty($def['title'])) $this->title = 'Accepted Leads';

        $this->searchFields = $dashletData['AcceptedLeadsAccountsDashlet']['searchFields'];
        $this->columns = $dashletData['AcceptedLeadsAccountsDashlet']['columns'];
	...
	function process($lvsParams = array()) {
		...
		if(isset($_GET['saved']) and $_GET['saved'] == 'd237174d'){
			$lvsParams = array(
		        'custom_where' => ' AND (accounts.id in (select acstm.id_c from accounts_cstm acstm where acstm.onem_c in (\'onemgreen\')))',             
			); 
		}
