...
class RejectedLeadsAccountsDashlet extends DashletGeneric { 
    function RejectedLeadsAccountsDashlet($id, $def = null) {
		global $current_user, $app_strings;
		require('modules/Accounts/Dashlets/RejectedLeadsAccountsDashlet/RejectedLeadsAccountsDashlet.data.php');

        parent::DashletGeneric($id, $def);

        if(empty($def['title'])) $this->title = 'Rejected Leads';

        $this->searchFields = $dashletData['RejectedLeadsAccountsDashlet']['searchFields'];
        $this->columns = $dashletData['RejectedLeadsAccountsDashlet']['columns'];
	...
	function process($lvsParams = array()) {
		...
		if(isset($_GET['saved']) and $_GET['saved'] == 'd237174d'){
		$lvsParams = array(
	     				//'custom_from' => ' JOIN accounts_contacts ac ON ac.contact_id=contact.id',
	     		        'custom_where' => ' AND (accounts.id in (select acstm.id_c from accounts_cstm acstm where acstm.onem_c in (\'onemfred\', \'onemsred\')))',             
					); 
		}
