...
class PendingAccountsDashlet extends DashletGeneric { 
    function PendingAccountsDashlet($id, $def = null) {
		global $current_user, $app_strings;
		require('modules/Accounts/Dashlets/PendingAccountsDashlet/PendingAccountsDashlet.data.php');

        parent::DashletGeneric($id, $def);

        if(empty($def['title'])) $this->title = 'Pending Accounts';

        $this->searchFields = $dashletData['PendingAccountsDashlet']['searchFields'];
        $this->columns = $dashletData['PendingAccountsDashlet']['columns'];
	...
	function process($lvsParams = array()) {
		...
		$lvsParams = array(
	     		        ...
	     		        'custom_where' => ' AND (accounts.id in (select acstm.id_c from accounts_cstm acstm where acstm.accsigned_c in (\'accorange\')))',             
					); 
		}
