...
class PendingContactsAccountsDashlet extends DashletGeneric { 
    function PendingContactsAccountsDashlet($id, $def = null) {
		global $current_user, $app_strings;
		require('modules/Accounts/Dashlets/PendingContactsAccountsDashlet/PendingContactsAccountsDashlet.data.php');

        parent::DashletGeneric($id, $def);

        if(empty($def['title'])) $this->title = 'Pending Contacts';

        $this->searchFields = $dashletData['PendingContactsAccountsDashlet']['searchFields'];
        $this->columns = $dashletData['PendingContactsAccountsDashlet']['columns'];
	...
	function process($lvsParams = array()) {
		...
		$lvsParams = array(
	     		        ...
	     		        'custom_where' => ' AND (accounts.id in (select acstm.id_c from accounts_cstm acstm where acstm.contact_c in (\'contactforange\', \'contactsorange\', \'contacttorange\')))',             
					); 
		}
