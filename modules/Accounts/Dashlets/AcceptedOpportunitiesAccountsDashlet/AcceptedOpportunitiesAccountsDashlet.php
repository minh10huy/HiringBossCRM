...
class AcceptedOpportunitiesAccountsDashlet extends DashletGeneric { 
    function AcceptedOpportunitiesAccountsDashlet($id, $def = null) {
		global $current_user, $app_strings;
		require('modules/Accounts/Dashlets/AcceptedOpportunitiesAccountsDashlet/AcceptedOpportunitiesAccountsDashlet.data.php');

        parent::DashletGeneric($id, $def);

        if(empty($def['title'])) $this->title = 'Accepted Opportunities';

        $this->searchFields = $dashletData['AcceptedOpportunitiesAccountsDashlet']['searchFields'];
        $this->columns = $dashletData['AcceptedOpportunitiesAccountsDashlet']['columns'];
	...
	function process($lvsParams = array()) {
		...
		if(isset($_GET['saved']) and $_GET['saved'] == '86826629'){
			$lvsParams = array(
		        'custom_where' => ' AND (accounts.id in (select acstm.id_c from accounts_cstm acstm where acstm.opplist_c in (\'ofgreen\', \'osgreen\')))',             
			);
		}
