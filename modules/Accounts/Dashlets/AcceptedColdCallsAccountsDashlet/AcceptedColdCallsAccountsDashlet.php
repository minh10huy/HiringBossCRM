...
class AcceptedColdCallsAccountsDashlet extends DashletGeneric { 
    function AcceptedColdCallsAccountsDashlet($id, $def = null) {
		global $current_user, $app_strings;
		require('modules/Accounts/Dashlets/AcceptedColdCallsAccountsDashlet/AcceptedColdCallsAccountsDashlet.data.php');

        parent::DashletGeneric($id, $def);

        if(empty($def['title'])) $this->title = 'Accepted Cold Calls';

        $this->searchFields = $dashletData['AcceptedColdCallsAccountsDashlet']['searchFields'];
        $this->columns = $dashletData['AcceptedColdCallsAccountsDashlet']['columns'];
	...
	function process($lvsParams = array()) {
		...
		if(isset($_GET['saved']) and $_GET['saved'] == '570c0afa'){
			$lvsParams = array(
				'custom_where' => ' AND (accounts.id in (select acstm.id_c from accounts_cstm acstm where acstm.coldcalls_c in (\'ccfgreen\', \'ccsgreen\', \'cctgreen\')))',           
			); 
		}
