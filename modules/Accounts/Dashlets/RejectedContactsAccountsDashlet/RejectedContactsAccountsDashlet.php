...
class RejectedContactsAccountsDashlet extends DashletGeneric { 
    function RejectedContactsAccountsDashlet($id, $def = null) {
		global $current_user, $app_strings;
		require('modules/Accounts/Dashlets/RejectedContactsAccountsDashlet/RejectedContactsAccountsDashlet.data.php');

        parent::DashletGeneric($id, $def);

        if(empty($def['title'])) $this->title = 'Rejected Contacts';

        $this->searchFields = $dashletData['RejectedContactsAccountsDashlet']['searchFields'];
        $this->columns = $dashletData['RejectedContactsAccountsDashlet']['columns'];
	...
	function process($lvsParams = array()) {
		...
		$lvsParams = array(
	     		        ...
	     		        'custom_where' => ' AND (accounts.id in (select acstm.id_c from accounts_cstm acstm where acstm.contact_c in (\'contactred\')))',             
					); 
		}
