...
class AcceptedNegotiationAccountsDashlet extends DashletGeneric { 
    function AcceptedNegotiationAccountsDashlet($id, $def = null) {
		global $current_user, $app_strings;
		require('modules/Accounts/Dashlets/AcceptedNegotiationAccountsDashlet/AcceptedNegotiationAccountsDashlet.data.php');

        parent::DashletGeneric($id, $def);

        if(empty($def['title'])) $this->title = 'Accepted Negotiation';

        $this->searchFields = $dashletData['AcceptedNegotiationAccountsDashlet']['searchFields'];
        $this->columns = $dashletData['AcceptedNegotiationAccountsDashlet']['columns'];
	...
	function process($lvsParams = array()) {
		...
		if(isset($_GET['saved']) and $_GET['saved'] == '2ac7796e'){
			$lvsParams = array(
		        'custom_where' => ' AND (accounts.id in (select acstm.id_c from accounts_cstm acstm where acstm.twom_c in (\'tmgreen\')))',             
			); 
		}
