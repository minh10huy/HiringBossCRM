...
class AcceptedAccountsDashlet extends DashletGeneric { 
    function AcceptedAccountsDashlet($id, $def = null) {
		global $current_user, $app_strings;
		require('modules/Accounts/Dashlets/AcceptedAccountsDashlet/AcceptedAccountsDashlet.data.php');

        parent::DashletGeneric($id, $def);

        if(empty($def['title'])) $this->title = 'Accepted Accounts';

        $this->searchFields = $dashletData['AcceptedAccountsDashlet']['searchFields'];
        $this->columns = $dashletData['AcceptedAccountsDashlet']['columns'];
	...
	function process($lvsParams = array()) {
		...
		if(isset($_GET['saved']) and $_GET['saved'] == '71cb0f85'){
			$lvsParams = array(
		        'custom_where' => ' AND (accounts.id in (select acstm.id_c from accounts_cstm acstm where acstm.accsigned_c in (\'accgreen\')))',             
			); 
		}
