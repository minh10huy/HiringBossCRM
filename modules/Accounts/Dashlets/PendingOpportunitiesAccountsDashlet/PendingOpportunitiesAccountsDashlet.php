...
class PendingOpportunitiesAccountsDashlet extends DashletGeneric { 
    function PendingOpportunitiesAccountsDashlet($id, $def = null) {
		global $current_user, $app_strings;
		require('modules/Accounts/Dashlets/PendingOpportunitiesAccountsDashlet/PendingOpportunitiesAccountsDashlet.data.php');

        parent::DashletGeneric($id, $def);

        if(empty($def['title'])) $this->title = 'Pending Opportunities';

        $this->searchFields = $dashletData['PendingOpportunitiesAccountsDashlet']['searchFields'];
        $this->columns = $dashletData['PendingOpportunitiesAccountsDashlet']['columns'];
	...
	function process($lvsParams = array()) {
		...
		$lvsParams = array(
	     		        ...
	     		        'custom_where' => ' AND (accounts.id in (select acstm.id_c from accounts_cstm acstm where acstm.opplist_c in (\'oorange\')))',             
					); 
		}
