...
class RejectedNegotiationAccountsDashlet extends DashletGeneric { 
    function RejectedNegotiationAccountsDashlet($id, $def = null) {
		global $current_user, $app_strings;
		require('modules/Accounts/Dashlets/RejectedNegotiationAccountsDashlet/RejectedNegotiationAccountsDashlet.data.php');

        parent::DashletGeneric($id, $def);

        if(empty($def['title'])) $this->title = 'Rejected Negotiation';

        $this->searchFields = $dashletData['RejectedNegotiationAccountsDashlet']['searchFields'];
        $this->columns = $dashletData['RejectedNegotiationAccountsDashlet']['columns'];
	...
	function process($lvsParams = array()) {
		...
		$lvsParams = array(
	     		        ...
	     		        'custom_where' => ' AND (accounts.id in (select acstm.id_c from accounts_cstm acstm where acstm.twom_c in (\'tmfred\', \'tmsred\')))',             
					); 
		}
