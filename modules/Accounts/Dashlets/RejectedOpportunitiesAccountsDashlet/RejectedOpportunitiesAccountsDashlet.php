...
class RejectedOpportunitiesAccountsDashlet extends DashletGeneric { 
    function RejectedOpportunitiesAccountsDashlet($id, $def = null) {
		global $current_user, $app_strings;
		require('modules/Accounts/Dashlets/RejectedOpportunitiesAccountsDashlet/RejectedOpportunitiesAccountsDashlet.data.php');

        parent::DashletGeneric($id, $def);

        if(empty($def['title'])) $this->title = 'Rejected Opportunities';

        $this->searchFields = $dashletData['RejectedOpportunitiesAccountsDashlet']['searchFields'];
        $this->columns = $dashletData['RejectedOpportunitiesAccountsDashlet']['columns'];
	...
	function process($lvsParams = array()) {
		...
		$lvsParams = array(
	     		        ...
	     		        'custom_where' => ' AND (accounts.id in (select acstm.id_c from accounts_cstm acstm where acstm.opplist_c in (\'ofred\', \'osred\')))',             
					); 
		}
