	...
	function handleSave($prefix,$redirect=true, $useRequired=false) {
		...
		if(strpos($emailTemplateBodyHtml, "\"{$fileBasePath}")) {
			...
			foreach($matches[0] as $match) {
				...
				if($filenameUndecoded != pathinfo($filenameUndecoded, PATHINFO_BASENAME)) {
				    // don't allow paths there
				    $emailTemplateBodyHtml = str_replace("{$sugar_config['cache_dir']}images/{$filenameUndecoded}", "", $emailTemplateBodyHtml);
				    continue;
				}
				$cid = $filename;
