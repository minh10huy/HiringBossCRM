...
function handleSave($prefix,$redirect=true, $useRequired=false) {
	...
	$query = "select onem_c from accounts_cstm where id_c='".$_POST[$prefix.'parent_id']."'";
	$r = $focus->db->query($query);
	while($a = $focus->db->fetchByAssoc($r)){
	    $b = $a['onem_c'];
	}
	if($b == '_none'){
		if($_POST[$prefix.'status'] == '_none'){
			$query = "update accounts_cstm set contact_c = 'contactgreen' where id_c ='".$_POST[$prefix.'parent_id']."'";
			$focus->db->query($query);
			$query = "update accounts_cstm set onem_c = 'onemsorange' where id_c ='".$_POST[$prefix.'parent_id']."'";
			$focus->db->query($query);
		}else {
			$query = "update accounts_cstm set onem_c = '".$_POST[$prefix.'status']."' where id_c = '".$_POST[$prefix.'parent_id']."'";
			$focus->db->query($query);
		}
	}else { 
		$query = "select opplist_c from accounts_cstm where id_c='".$_POST[$prefix.'parent_id']."'";
		$r = $focus->db->query($query);
		while($a = $focus->db->fetchByAssoc($r)){
		    $b = $a['opplist_c'];
		}
		if($b == '_none'){
			//update meetings
			if($_POST[$prefix.'status'] == '_none'){
				$query = "update accounts_cstm set onem_c = 'onemorange' where id_c ='".$_POST[$prefix.'parent_id']."'";
				$focus->db->query($query);
				$query = "update accounts_cstm set contact_c = 'contactgreen' where id_c ='".$_POST[$prefix.'parent_id']."'";
				$focus->db->query($query);
				$_POST[$prefix.'status'] = 'onemorange';
			}else{
				$query = "update accounts_cstm set onem_c = '".$_POST[$prefix.'status']."' where id_c = '".$_POST[$prefix.'parent_id']."'";
				$focus->db->query($query);
			}
		}else {
			$query = "update accounts_cstm set opplist_c ='ofgreen' where id_c ='".$_POST[$prefix.'parent_id']."'";
			$focus->db->query($query);
			$query = "update accounts_cstm set twom_c = 'tmorange' where id_c ='".$_POST[$prefix.'parent_id']."'";
			$focus->db->query($query);
		}
	}
