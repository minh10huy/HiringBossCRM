...
function handleSave($prefix,$redirect=true,$useRequired=false) {
	...
	global $timedate;
	$callc = new Account();
	$query = "select coldcalls_c from accounts_cstm where id_c='".$_POST[$prefix.'parent_id']."'";
		$r = $callc->db->query($query);
		while($a = $callc->db->fetchByAssoc($r)){
	    	$b = $a['coldcalls_c'];
	    }
	if($b == '_none'){
		if($_POST[$prefix.'status'] == '_none'){
			$query = "update accounts_cstm set coldcalls_c = 'ccorange' where id_c = '".$_POST[$prefix.'parent_id']."'";
			$callc->db->query($query);
			$_POST[$prefix.'status'] = 'ccorange';
		}elseif($_POST[$prefix.'status'] == 'ccfgreen') {
			$query = "update accounts_cstm set coldcalls_c = '".$_POST[$prefix.'status']."' where id_c = '".$_POST[$prefix.'parent_id']."'";
			$callc->db->query($query);
			$query = "update accounts_cstm set contact_c = 'contactforange' where id_c = '".$_POST[$prefix.'parent_id']."'";
			$callc->db->query($query);
		}elseif($_POST[$prefix.'status'] == 'ccsgreen'){
			$query = "update accounts_cstm set coldcalls_c = '".$_POST[$prefix.'status']."' where id_c = '".$_POST[$prefix.'parent_id']."'";
			$callc->db->query($query);
			$query = "update accounts_cstm set contact_c = 'contactsorange' where id_c = '".$_POST[$prefix.'parent_id']."'";
			$callc->db->query($query);
		}elseif($_POST[$prefix.'status'] == 'cctgreen'){
			$query = "update accounts_cstm set coldcalls_c = '".$_POST[$prefix.'status']."' where id_c = '".$_POST[$prefix.'parent_id']."'";
			$callc->db->query($query);
			$query = "update accounts_cstm set contact_c = 'contacttorange' where id_c = '".$_POST[$prefix.'parent_id']."'";
			$callc->db->query($query);
		}else {
			$query = "update accounts_cstm set coldcalls_c = '".$_POST[$prefix.'status']."' where id_c = '".$_POST[$prefix.'parent_id']."'";
			$callc->db->query($query);
		...
		$query = "select contact_c from accounts_cstm where id_c='".$_POST[$prefix.'parent_id']."'";
		$r = $callc->db->query($query);
		while($a = $callc->db->fetchByAssoc($r)){
	    	$b = $a['contact_c'];
	    }
